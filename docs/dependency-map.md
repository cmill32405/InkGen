...
| `raster_renderer.py -> style.py/gradients.py/NumPy/Pillow` | Direct raster output segments line dashes in neutral logical units, constructs bounded line-cap geometry in supersampled pixel space, projects the neutral gradient axis in bounded NumPy tiles, evaluates path fills with a bounded nonzero-winding scanline mask, and uses Pillow for clipping and alpha composition. | Raster output must not become an SVG/PDF intermediary, reinterpret dash phase after pixel scaling, apply caps to unsupported primitives, renormalize clipped geometry, substitute even-odd path semantics, or allocate unbounded paint/interpolation work. |
