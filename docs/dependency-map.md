...
| `raster_renderer.py -> gradients.py/NumPy/Pillow` | Direct raster output projects the neutral gradient axis in bounded NumPy tiles, evaluates path fills with a bounded nonzero-winding scanline mask, and uses Pillow for clipping and alpha composition. | Raster output must not become an SVG/PDF intermediary, renormalize clipped geometry, substitute even-odd path semantics, or allocate unbounded interpolation grids. |
