...
extraction truth, and grammar truth when a layer contains repeated semantic
labels.
...
- `src/InkGen/pdf_generator.py`: PDF document traversal now reads the layer's
  stored groups rather than the lossy label lookup.
...
| Failure-mode | yes | Existing non-PDF group failure must remain. | `test_document_pdf_rejects_non_pdf_child_in_standard_group` |
...
| Security/adversarial | no | No untrusted path, subprocess, network, template, archive, or active-content handling changed. | Not applicable |
