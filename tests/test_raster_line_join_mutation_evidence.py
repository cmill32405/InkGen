"""Freshness and completeness checks for raster line-join P15 mutation evidence."""
...
MANIFEST_PATH = ROOT / "tests" / "mutation" / "raster_line_join_p15_evidence.json"


@pytest.mark.condition("RASTER-LINE-JOIN-P15")
def test_raster_line_join_mutation_certificate_matches_final_source_and_database() -> None:
    """P15: The retained complete campaign remains source-fresh and auditable."""
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    campaign = manifest["campaign"]
    database_path = ROOT / manifest["database"]["path"]

    assert manifest["condition"] == "RASTER-LINE-JOIN-P15"
    assert campaign == {
        "generated_candidates": 5360,
        "selected_work_items": 333,
        "completed_results": 333,
        "killed": 327,
        "survived": 6,
        "equivalent_survivors": 6,
        "worker_errors": 0,
        "worker_timeouts": 0,
        "raw_mutation_coverage": pytest.approx(327 / 333),
