"""Freshness and completeness checks for raster line-dash P13 mutation evidence."""
...
MANIFEST_PATH = ROOT / "tests" / "mutation" / "raster_line_dash_p13_evidence.json"


@pytest.mark.condition("RASTER-LINE-DASH-P13")
def test_raster_line_dash_mutation_certificate_matches_final_source_and_database() -> None:
    """P13: The retained complete campaign remains source-fresh and auditable."""
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    campaign = manifest["campaign"]
    database_path = ROOT / manifest["database"]["path"]

    assert manifest["condition"] == "RASTER-LINE-DASH-P13"
    assert campaign == {
        "generated_candidates": 4109,
        "selected_work_items": 231,
        "completed_results": 231,
        "killed": 218,
        "survived": 13,
        "equivalent_survivors": 13,
        "worker_errors": 0,
        "worker_timeouts": 0,
        "raw_mutation_coverage": pytest.approx(218 / 231),
