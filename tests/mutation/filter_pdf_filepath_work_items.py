"""Filter Cosmic Ray work items to PDF filepath rows."""
...
AND (
  (
    definition_name = 'create_pdf'
    AND start_pos_row BETWEEN 851 AND 855
  )
  OR (
    definition_name = '_normalize_output_filepath'
    AND start_pos_row BETWEEN 1013 AND 1027
  )
)
AND (
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF filepath validation checks."""
