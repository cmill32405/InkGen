"""Filter Cosmic Ray work items to FLOW-DOCUMENT-DRAWING-POINTS-P2 rows."""
...
AND (
  (
    ...
    AND start_pos_row = 498
  )
  OR (
    definition_name = '_component_vml'
    AND start_pos_row = 524
  )
  OR (
    definition_name IN (
      '_materialized_points',
      '_materialized_point',
      '_materialized_coordinate'
    )
    AND start_pos_row BETWEEN 531 AND 560
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to flow-document point-surface checks."""
