"""Filter Cosmic Ray work items to FLOW-DOCUMENT-RTF-UNICODE-P2 rows."""
...
AND (
  (
    definition_name = 'to_rtf'
    AND start_pos_row BETWEEN 132 AND 140
  )
  OR (
    definition_name = '_paragraph_rtf'
    AND start_pos_row BETWEEN 307 AND 323
  )
  OR (
    definition_name = '_rtf_escape'
    AND start_pos_row BETWEEN 666 AND 683
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to RTF Unicode escaping checks."""
