"""Filter Cosmic Ray work items to PDF-DOC-DEEP-OUTLINE-P3 rows."""
...
AND (
  (
    definition_name = 'add_outline'
    AND start_pos_row BETWEEN 1730 AND 1733
  )
  OR (
    definition_name = '_shift_pdf_page_metadata_for_removal'
    AND start_pos_row = 1997
  )
  OR (
    definition_name = '_outlines_with_valid_parent_chains'
    AND start_pos_row BETWEEN 2091 AND 2095
  )
  OR (
    definition_name = '_outline_objects'
    AND start_pos_row BETWEEN 2119 AND 2148
  )
  OR (
    definition_name = '_outline_descendant_count'
    AND start_pos_row BETWEEN 2160 AND 2161
  )
  OR (
    definition_name = 'create_from_dict'
    AND start_pos_row BETWEEN 2588 AND 2598
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to deep outline proof-critical rows."""
