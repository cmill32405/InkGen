"""Filter Cosmic Ray work items to PDF-FONT-STANDARD-P2 rows."""
...
AND (
  (
    definition_name IN ('font_resource_name', 'resource_name_for_style', 'resource_name_for_base_font', 'resources')
    AND start_pos_row BETWEEN 80 AND 105
  )
  OR (
    definition_name = '_pdf_base_font_for_style'
    AND start_pos_row BETWEEN 170 AND 204
  )
  OR (
    definition_name = 'generate_pdf'
    AND start_pos_row BETWEEN 873 AND 888
  )
  OR (
    definition_name IN ('to_pdf_bytes', '_render_page_content')
    AND start_pos_row BETWEEN 1018 AND 1086
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF Standard font work items."""
