"""Filter Cosmic Ray work items to PDF-GROUP-CLIP-P3 rows."""
...
AND (
  (
    definition_name = '_coerce_pdf_clip_rect'
    AND start_pos_row BETWEEN 531 AND 548
  )
  OR (
    definition_name IN (
      '__init__',
    ...
    AND (
      start_pos_row BETWEEN 1700 AND 1727
      OR start_pos_row BETWEEN 1730 AND 1739
      OR start_pos_row = 1769
      OR start_pos_row BETWEEN 1785 AND 1798
    )
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF group clipping proof-critical rows."""
