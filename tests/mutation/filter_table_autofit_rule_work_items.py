"""Filter Cosmic Ray work items to the TABLE-AUTOFIT-RULE-P2 rows."""
...
(
  ...
  AND start_pos_row BETWEEN 313 AND 318
)
OR (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_normalize_autofit_rule'
  AND start_pos_row BETWEEN 450 AND 454
)
)
...
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_normalize_autofit_rule'
  AND start_pos_row = 450
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to TABLE-AUTOFIT-RULE-P2 work items."""
