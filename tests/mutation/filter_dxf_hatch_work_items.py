"""Filter Cosmic Ray work items to DXF-HATCH-P3 rows."""
...
AND (
  (definition_name = '_component_to_entities' AND start_pos_row BETWEEN 197 AND 219)
  OR (definition_name = '_closed_shape_entities' AND start_pos_row BETWEEN 231 AND 242)
  OR (definition_name = '_style_has_fill' AND start_pos_row BETWEEN 368 AND 370)
  OR (definition_name = '_circle_hatch_points' AND start_pos_row BETWEEN 373 AND 381)
  OR (definition_name = '_hatch_entity' AND start_pos_row BETWEEN 384 AND 433)
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to DXF HATCH proof-critical rows."""
