"""Filter Cosmic Ray work items to the REGPOLY-P1 proof-critical rows."""
...
(
  ...
  AND start_pos_row BETWEEN 985 AND 990
)
OR (
  module_path = 'src/InkGen/component.py'
  AND definition_name = '_get_points'
  AND start_pos_row BETWEEN 1036 AND 1043
)
OR (
  module_path = 'src/InkGen/component.py'
  AND definition_name = 'sides'
  AND start_pos_row BETWEEN 1071 AND 1076
)
OR (
  module_path = 'src/InkGen/component.py'
  AND definition_name = 'radius'
  AND start_pos_row BETWEEN 1103 AND 1107
)
OR (
  module_path = 'src/InkGen/component.py'
  AND definition_name = 'corner_radius'
  AND start_pos_row BETWEEN 1129 AND 1134
)
OR (
...
OR (
  ...
  AND start_pos_row = 574
)
OR (
  module_path = 'src/InkGen/dxf_generator.py'
  AND definition_name = '_component_to_entities'
  AND start_pos_row BETWEEN 100 AND 102
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to REGPOLY-P1 proof-critical work items."""
