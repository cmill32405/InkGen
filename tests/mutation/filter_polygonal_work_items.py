"""Filter Cosmic Ray work items to the POLYGON-P1 proof-critical rows."""
...
(
  (
    ...
    AND definition_name IN ('__init__', '_polygon_check', '_create_valid_polygon', 'bbox', 'points', 'polygon', 'convex_hull')
    AND start_pos_row BETWEEN 848 AND 975
  )
  OR (
    module_path = 'src/InkGen/component.py'
    AND definition_name = 'points'
    AND start_pos_row BETWEEN 959 AND 972
  )
  OR (
    module_path = 'src/InkGen/svg_generator.py'
    AND definition_name IN ('generate_svg', 'parameters', 'create_from_dict')
    AND start_pos_row BETWEEN 752 AND 809
  )
  OR (
    module_path = 'src/InkGen/pdf_generator.py'
    AND definition_name IN ('generate_pdf', 'parameters', 'create_from_dict')
    AND start_pos_row BETWEEN 627 AND 642
  )
  OR (
  ...
  OR (
    ...
    AND start_pos_row BETWEEN 99 AND 100
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to POLYGON-P1 proof-critical work items."""
