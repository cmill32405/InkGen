"""Filter Cosmic Ray work items to DOCUMENT-MODEL-IDENTIFIER-P2 rows."""
...
AND (
  (
    ...
    AND start_pos_row BETWEEN 202 AND 203
  )
  OR (
    definition_name = 'group'
    AND start_pos_row BETWEEN 253 AND 254
  )
  OR (
    definition_name = '_layer_identification_lookup'
    AND start_pos_row BETWEEN 373 AND 374
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to document-model identifier checks."""
