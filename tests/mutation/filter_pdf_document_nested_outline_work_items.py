"""Filter Cosmic Ray work items to PDF-DOC-NESTED-OUTLINE-P3 rows."""
...
AND (
  (
    ...
    AND start_pos_row BETWEEN 77 AND 87
  )
  OR (
    definition_name IN (
    ...
    AND start_pos_row BETWEEN 388 AND 430
  )
  OR (
    definition_name = '_pdf_optional_sequence'
    AND start_pos_row BETWEEN 875 AND 882
  )
  OR (
    definition_name IN (
      'add_outline',
    ...
    AND (
      start_pos_row BETWEEN 1672 AND 1704
      OR start_pos_row BETWEEN 1838 AND 1852
      OR start_pos_row BETWEEN 1891 AND 1912
      OR start_pos_row BETWEEN 1973 AND 2038
    )
  )
  OR (
    definition_name = 'to_pdf_bytes'
    AND (
      start_pos_row BETWEEN 2327 AND 2340
    )
  )
  OR (
    definition_name = 'create_from_dict'
    AND start_pos_row BETWEEN 2455 AND 2464
  )
  OR (
    definition_name = 'parameters'
    AND start_pos_row BETWEEN 2520 AND 2521
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF nested outline proof-critical rows."""
