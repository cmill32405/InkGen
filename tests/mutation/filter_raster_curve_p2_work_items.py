"""Filter Cosmic Ray work items to the raster curve P2 slice."""
...
AND (
  (definition_name = '_validated_components' AND start_pos_row BETWEEN 217 AND 222)
  OR (definition_name = '_validate_render_domain' AND start_pos_row BETWEEN 274 AND 278)
  OR (definition_name = '_render_component' AND start_pos_row BETWEEN 312 AND 330)
...
def filter_work_items(db_path: Path) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to curve proof obligations."""
