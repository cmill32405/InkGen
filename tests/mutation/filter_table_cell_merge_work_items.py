"""Filter Cosmic Ray work items to the TABLE-CELL-MERGE-P2 rows."""
...
(
  ...
  AND start_pos_row BETWEEN 328 AND 340
)
OR (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_normalize_cell_coordinate'
  AND start_pos_row BETWEEN 466 AND 486
)
)
...
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_normalize_cell_coordinate'
  AND start_pos_row = 468
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to TABLE-CELL-MERGE-P2 work items."""
