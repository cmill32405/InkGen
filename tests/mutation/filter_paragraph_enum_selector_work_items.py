"""Filter Cosmic Ray work items to PARAGRAPH-ENUM-SELECTOR-P2 rows."""
...
AND (
  (
    ...
    AND start_pos_row = 49
  )
  OR (
    definition_name = 'create_from_dict'
    AND (
      start_pos_row BETWEEN 61 AND 66
      OR start_pos_row BETWEEN 410 AND 423
    )
  )
  OR (
    definition_name = 'alignment'
    AND start_pos_row BETWEEN 197 AND 198
  )
  OR (
    definition_name = 'line_spacing_rule'
    AND start_pos_row BETWEEN 269 AND 270
  )
  OR (
    definition_name = 'add_tab_stop'
    AND start_pos_row BETWEEN 324 AND 333
  )
  OR (
    definition_name IN ('_normalize_paragraph_alignment', '_normalize_line_spacing_rule')
    AND start_pos_row BETWEEN 540 AND 555
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to paragraph enum selector checks."""
