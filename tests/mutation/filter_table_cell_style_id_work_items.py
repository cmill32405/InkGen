"""Filter Cosmic Ray work items to the TABLE-CELL-STYLE-ID-P2 rows."""
...
(
  ...
  AND definition_name = '_normalize_style_id'
  AND start_pos_row BETWEEN 449 AND 453
)
OR (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_append_paragraph'
  AND start_pos_row BETWEEN 803 AND 816
)
)
...
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_append_paragraph'
  AND start_pos_row = 807
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to TABLE-CELL-STYLE-ID-P2 work items."""
