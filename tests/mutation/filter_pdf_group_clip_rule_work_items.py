"""Filter Cosmic Ray work items to PDF-GROUP-CLIP-RULE-P3 rows."""
...
AND (
  (
    definition_name = '_coerce_pdf_clip_rule'
    AND start_pos_row BETWEEN 652 AND 663
  )
  OR (
    definition_name IN (
      '__init__',
    ...
    AND (
      start_pos_row BETWEEN 1842 AND 1845
      OR start_pos_row BETWEEN 1888 AND 1898
      OR start_pos_row BETWEEN 1921 AND 1922
      OR start_pos_row BETWEEN 1962 AND 1963
      OR start_pos_row BETWEEN 1998 AND 2014
    )
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF group clip-rule proof-critical rows."""
