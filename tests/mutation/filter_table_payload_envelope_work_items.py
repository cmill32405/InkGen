"""Filter Cosmic Ray work items to the TABLE-PAYLOAD-ENVELOPE-P2 rows."""
...
(
  ...
  AND start_pos_row BETWEEN 310 AND 329
)
OR (
  module_path = 'src/InkGen/table.py'
  AND definition_name IN ('_normalize_payload_mapping', '_normalize_payload_sequence')
  AND start_pos_row BETWEEN 457 AND 468
)
)
...
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name IN ('_normalize_payload_mapping', '_normalize_payload_sequence')
  AND start_pos_row IN (457, 464)
)
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name = 'create_from_dict'
  AND start_pos_row = 314
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to TABLE-PAYLOAD-ENVELOPE-P2 work items."""
