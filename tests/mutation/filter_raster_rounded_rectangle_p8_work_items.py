"""Filter Cosmic Ray work items to the raster rounded-rectangle P8 slice."""
...
AND (
  definition_name = '_draw_rounded_rectangle'
  OR (
    definition_name = '_validate_render_domain'
    AND start_pos_row BETWEEN 288 AND 289
  )
  OR (
    definition_name = '_render_component'
    AND start_pos_row BETWEEN 345 AND 370
  )
)
...
def filter_work_items(db_path: Path) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to P8 rounded-rectangle obligations."""
