"""Filter Cosmic Ray work items to the TABLE-REQUIRED-FIELDS-P2 rows."""
...
(
  ...
  AND start_pos_row IN (317, 318, 324, 326, 331, 333, 349)
)
OR (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_required_payload_field'
  AND start_pos_row BETWEEN 487 AND 491
)
)
...
AND NOT (
  module_path = 'src/InkGen/table.py'
  AND definition_name = '_required_payload_field'
  AND start_pos_row = 487
)
"""


def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to TABLE-REQUIRED-FIELDS-P2 work items."""
