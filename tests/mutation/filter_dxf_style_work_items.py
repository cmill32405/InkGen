"""Filter Cosmic Ray work items to DXF style-emission proof-critical rows."""
...
AND (
  definition_name IN ('_dxf_style_pairs', '_dxf_true_color', '_dxf_lineweight')
  OR (definition_name = '_component_to_entities' AND start_pos_row IN (197, 200, 204, 207, 210, 214))
  OR (definition_name = '_line_entity' AND start_pos_row IN (285))
  OR (definition_name = '_lwpolyline_entity' AND start_pos_row IN (306))
  OR (definition_name = '_circle_entity' AND start_pos_row IN (339))
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to DXF style-emission work items."""
