"""Filter Cosmic Ray work items to PDF-DOC-HIGHLIGHT-ANNOTATION-P3 rows."""
...
AND (
  (
    definition_name = '_PDFHighlightAnnotation'
    AND start_pos_row BETWEEN 143 AND 151
  )
  OR (
    definition_name IN (
      '_coerce_pdf_annotation_color'
    )
    AND start_pos_row BETWEEN 494 AND 520
  )
  OR (
    definition_name IN (
      'add_highlight_annotation',
      'clear_highlight_annotations',
      'highlight_annotations',
      '_shift_pdf_page_metadata_for_insert',
      '_shift_pdf_page_metadata_for_removal',
      '_highlight_annotation_payload',
      '_highlight_annotations_by_page',
      '_highlight_annotation_object'
    )
    AND (
      start_pos_row BETWEEN 1965 AND 1987
      OR start_pos_row BETWEEN 2091 AND 2099
      OR start_pos_row BETWEEN 2178 AND 2188
      OR start_pos_row BETWEEN 2447 AND 2474
    )
  )
  OR (
    definition_name = 'to_pdf_bytes'
    AND (
      start_pos_row BETWEEN 2599 AND 2612
      OR start_pos_row BETWEEN 2643 AND 2647
    )
  )
  OR (
    definition_name = 'create_from_dict'
    AND start_pos_row BETWEEN 2834 AND 2841
  )
  OR (
    definition_name = 'parameters'
    AND start_pos_row BETWEEN 2880 AND 2883
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF highlight annotation proof-critical rows."""
