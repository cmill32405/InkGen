"""Filter Cosmic Ray work items to PDF-DOC-OUTLINE-STATE-P3 rows."""
...
AND (
  (
    definition_name = '_coerce_pdf_outline_expanded'
    AND start_pos_row = 416
  )
  OR (
    definition_name = 'add_outline'
    AND start_pos_row = 1722
  )
  OR (
    definition_name = '_outline_entry_payload'
    AND start_pos_row = 2084
  )
  OR (
    definition_name = '_outline_objects'
    AND start_pos_row = 2131
  )
  OR (
    definition_name = 'create_from_dict'
    AND start_pos_row = 2597
  )
  OR (
    definition_name = 'parameters'
    AND start_pos_row = 2665
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to PDF outline state proof-critical rows."""
