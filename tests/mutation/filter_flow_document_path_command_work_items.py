"""Filter Cosmic Ray work items to flow-document path command rows."""
...
AND (
  (
    ...
    AND start_pos_row BETWEEN 572 AND 573
  )
  OR (
    definition_name = '_path_commands_from_payload'
    AND start_pos_row BETWEEN 577 AND 593
  )
)
...
def filter_work_items(db_path: Path, *, clear_results: bool) -> tuple[int, int]:
    """Restrict a Cosmic Ray database to path command validation checks."""
