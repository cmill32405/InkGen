"""Freshness and completeness checks for raster path-dash P18 evidence."""
...
MANIFEST_PATH = ROOT / "tests" / "mutation" / "raster_path_dash_p18_evidence.json"


@pytest.mark.condition("RASTER-PATH-DASH-P18")
def test_raster_path_dash_mutation_certificate_matches_final_source_and_database() -> None:
    """P18: The retained complete campaign remains source-fresh and auditable."""
    manifest = json.loads(MANIFEST_PATH.read_text(encoding="utf-8"))
    campaign = manifest["campaign"]
    database_path = ROOT / manifest["database"]["path"]

    assert manifest["condition"] == "RASTER-PATH-DASH-P18"
    assert campaign == {
        "generated_candidates": 7220,
        "selected_work_items": 736,
        "completed_results": 736,
        "killed": 670,
        "survived": 66,
        "equivalent_survivors": 66,
        "worker_errors": 0,
        "worker_timeouts": 0,
        "raw_mutation_coverage": pytest.approx(670 / 736),
